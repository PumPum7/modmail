...
async fn get_threads(
    ...
    let threads = match threads_result {
        ...
        Err(_) => {
            return (
                ...
                Json(serde_json::json!({ "error": "Failed to fetch threads" })),
            )
                .into_response()
        }
    };
    ...
    let total_count_result: Result<i64, sqlx::Error> =
        sqlx::query_scalar("SELECT COUNT(*) FROM threads WHERE guild_id = $1")
            .bind(guild_id)
            .fetch_one(&pool)
            .await;
    ...
    let total_count = match total_count_result {
        ...
        Err(_) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({ "error": "Failed to count threads" })),
            )
                .into_response()
        }
    };
...
async fn create_thread(
    ...
    match new_thread_result {
        Ok(new_thread) => (StatusCode::OK, Json(new_thread)).into_response(),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": "Failed to create thread" })),
        )
            .into_response(),
    }
}
...
async fn get_thread(
    ...
    let total_count = match total_count_result {
        ...
        Err(_) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({ "error": "Failed to count messages" })),
            )
                .into_response()
        }
    };
...
async fn close_thread(
    ...
    match updated_thread_result {
        Ok(updated_thread) => {
            ...
            return (StatusCode::OK, Json(updated_thread)).into_response();
        }
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": "Failed to close thread" })),
        )
            .into_response(),
    }
}
...
async fn add_message_to_thread(
    ...
    if link_result.is_err() {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": "Failed to link message to thread" })),
        )
            .into_response();
    }

    (StatusCode::OK, Json(new_message)).into_response()
}
...
async fn update_thread_urgency(
    ...
    match updated_thread_result {
        Ok(updated_thread) => (StatusCode::OK, Json(updated_thread)).into_response(),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({ "error": "Failed to update thread urgency" })),
        )
            .into_response(),
    }
}
