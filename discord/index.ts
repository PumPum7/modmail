    ...
    if (interaction.customId.startsWith('intro_modal_')) {
      ...
      try {
        ...
        if (result) {
          ...
          await interaction.reply({
            content: "✅ Thank you! Your modmail thread has been created. A moderator will respond to you shortly.",
            flags: MessageFlagsBitField.Flags.Ephemeral,
          });

          console.log(`Created thread ${thread.id} for user ${interaction.user.tag} (${interaction.user.id}) via intro form`);
        } else {
        ...
        await interaction.reply({
          content: "❌ There was an error processing your information. Please try contacting us again.",
          flags: MessageFlagsBitField.Flags.Ephemeral,
        });
      }
    }
