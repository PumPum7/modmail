...
async function addMessageToThread(threadId: number, authorId: string, authorTag: string, content: string): Promise<MessageData> {
...
async function deleteMacro(name: string): Promise<{ success: boolean; message: string }> {
    const response = await fetch(`${BACKEND_URL}/macros/${encodeURIComponent(name)}`, {
        method: 'DELETE',
    });
    ...
    try {
        ...
        if (!thread || thread.id === (await getThreadByUserId(userId))?.id) {
            const confirmEmbed = new EmbedBuilder()
                ...
                .setDescription('Your message has been sent to the moderators. They will respond as soon as possible.')
                .setTimestamp();
        ...
        try {
            await message.author.send('❌ There was an error processing your message. Please try again later.');
        } catch (dmError) {
